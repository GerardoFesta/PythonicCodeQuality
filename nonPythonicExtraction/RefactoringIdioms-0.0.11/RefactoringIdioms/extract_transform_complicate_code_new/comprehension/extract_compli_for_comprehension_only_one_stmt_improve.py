    ...
    try:
        ...
        for tree, class_name in ana_py.func_def_list:
            ...
            for ind, (for_node, assign_node, remove_ass_flag) in enumerate(new_code_list):
                ...
                assign_node_content = ast.get_source_segment(content, assign_node)
                for_node_content = ast.get_source_segment(content, for_node)
                # print(">>>>>assign_node_content:\n",[assign_node_content, for_node_content])
                if remove_ass_flag:
                    line_list = []

                    line_list.append([(assign_node.lineno,assign_node.col_offset), (assign_node.end_lineno,assign_node.end_col_offset)])
                    line_list.append([(for_node.lineno,for_node.col_offset), (for_node.end_lineno,for_node.end_col_offset)])
                    code_pair_list.append([class_name, me_name, [assign_node_content, for_node_content],
                                           ['', ast.unparse(new_code)],
                                           line_list])
                    # code_pair_list.append([class_name, me_name, [ast.unparse(assign_node), ast.unparse(for_node)],
                    #                        ['', ast.unparse(new_code)],
                    #                        line_list])
                else:
                    line_list = []
                    ...
                    #     [class_name, me_name, [ast.unparse(for_node)], [ast.unparse(new_code)], line_list])
                    code_pair_list.append(
                        [class_name, me_name, [for_node_content], [ast.unparse(new_code)], line_list])
                ...
                #     line_list = []
                #     line_list.append([for_node.lineno, for_node.end_lineno])
                #     code_pair_list.append([class_name,me_name,[ast.unparse(for_node)], [ast.unparse(new_code)],line_list])
...
def get_list_compreh_part_fragment(content):
    code_pair_list=[]
    ...
    try:
        ...
        # for tree, class_name in ana_py.func_def_list:
            ...
            for ind, (for_node, assign_node, remove_ass_flag) in enumerate(new_code_list):
                ...
                assign_node_content = ast.get_source_segment(content, assign_node)
                for_node_content = ast.get_source_segment(content, for_node)
                # print(">>>>>for_node_content:\n",for_node_content)
                if 1:
                # if target_node_str[-1]==ast.unparse(assign_node)+"\n"+ast.unparse(for_node):
                    if remove_ass_flag:
                        line_list = []

                        line_list.append([(assign_node.lineno,assign_node.col_offset), (assign_node.end_lineno,assign_node.end_col_offset)])
                        line_list.append([(for_node.lineno,for_node.col_offset), (for_node.end_lineno,for_node.end_col_offset)])
                        code_pair_list.append(['', me_name, [assign_node_content, for_node_content],
                                               ['', ast.unparse(new_code)],
                                               line_list])
                        # code_pair_list.append([class_name, me_name, [ast.unparse(assign_node), ast.unparse(for_node)],
                        #                        ['', ast.unparse(new_code)],
                        ...
                        #     [class_name, me_name, [ast.unparse(for_node)], [ast.unparse(new_code)], line_list])
                        code_pair_list.append(
                            ['', me_name, [for_node_content], [ast.unparse(new_code)], line_list])
