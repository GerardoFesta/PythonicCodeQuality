...
def file_replace(abs_path, each_file_code_list):
    ...
    for ind_group, (start_row, end_row, compli_code, simple_code, idiom, lineno) \
        ...
        if not prelineno:
            ...
            continue
        if start_row <= prelineno[
            1]:  # if start_row <>>= group_new_sorted_code_list[-1][0][0] and end_row <= group_new_sorted_code_list[-1][0][1]:
            ...
            for ind_each_code, each_code in enumerate(each_group_code):
                ...
                if len(each_group_code) == 1:
                    ...
                    if idiom == 'List Comprehension':
                        ...
                        code_pair_list = extract_compli_for_comprehension_only_one_stmt_improve.get_list_compreh_part_fragment(
                            "".join(code_fragment_list))

                        pass
                    elif idiom == 'Set Comprehension':
                        code_pair_list = extract_compli_for_comprehension_set_only_one_stmt.get_part_set_compreh(
                            "".join(code_fragment_list))
                        pass
                    elif idiom == 'Dict Comprehension':
                        ...
                        while 1:
                            try:
                                ...
                                code_pair_list = extract_compli_truth_value_test_code_remove_is_isnot.get_part_truth_value_test_code(
                                    "".join(code_fragment_list))
                                break
                            except:
                                k += 1
                        ...
                        while 1:
                            try:
                                ...
                                code_pair_list = transform_chained_comparison_compli_to_simple.get_part_chain_compare(
                                    "".join(code_fragment_list))
                                break
                            except:
                                k += 1
                    ...
                    #     simp_code = code_pair_list[0][-2]
                        ...
                        for ind_code_frag, e in enumerate(code_fragment_list):
                            if e != "\n":
                                code_fragment_list[ind_code_frag] = " " * count_space + code_fragment_list[
                                    ind_code_frag]
        ...
        for path, dir_list, file_list in os.walk(filepath):
            for file_name in file_list:
                ...
                if abs_path.endswith(".py"):
                    print(
                        f"************For File {abs_path.split('/')[-1]}, begin to find refactorable non-idiomatic code with Python idioms************")
